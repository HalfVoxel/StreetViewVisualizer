...
line_v2f line_vert (appdata_color v, float pixelWidth, out float4 outpos : SV_POSITION) {
	...
	float4 p2 = UnityObjectToClipPos(float4(v.vertex.xyz + v.normal*0.001, v.vertex.w));
#else
	float4 p1 = UnityObjectToClipPos(v.vertex);
    //float4 p2 = UnityObjectToClipPos(float4(v.vertex.xyz + v.normal*0.001, v.vertex.w));
#endif
	...
	float4 delta = (UnityObjectToClipPos(v.normal) - UnityObjectToClipPos(float3(0,0,0)))/p1.w;
